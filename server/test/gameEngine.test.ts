...
import { SEED_QUESTIONS } from '../src/services/SeedData';
...
async function runTests() {
  ...
  const room = await store.getRoom(testRoomCode);
  ...
  const participants = await store.getParticipantsByRoom(testRoomCode);
